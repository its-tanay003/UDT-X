"""UDT-X DGA & DNS Tunnelling Detection Engine CLI Entrypoint."""
...
logger = logging.getLogger("udtx.engines.dga_dns_tunnel.main")


def main() -> None:
    parser = argparse.ArgumentParser(
        description="UDT-X DGA & DNS Tunnelling Detection Engine"
    )
    parser.add_argument(
        "--kafka-brokers",
    ...
    parser.add_argument(
        "--group-id",
        default=os.getenv("KAFKA_GROUP_ID", "udtx-dga-dns-tunnel-engine"),
    )
    parser.add_argument(
        "--threshold",
        type=float,
    ...
    logger.info("Starting UDT-X DGA & DNS Tunnelling Detection Engine")
    ...
    try:
        ...
        logger.info("Shutting down DGA & DNS Tunnelling engine.")
        sys.exit(0)
