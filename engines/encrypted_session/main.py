"""UDT-X Encrypted-Session Anomaly Detection Engine CLI Entrypoint."""
...
logger = logging.getLogger("udtx.engines.encrypted_session.main")


def main() -> None:
    parser = argparse.ArgumentParser(
        description="UDT-X Encrypted-Session Anomaly Detection Engine"
    )
    parser.add_argument(
        "--kafka-brokers",
    ...
    parser.add_argument(
        "--group-id",
        default=os.getenv("KAFKA_GROUP_ID", "udtx-encrypted-session-engine"),
    )
    parser.add_argument(
        "--threshold",
        type=float,
    ...
    logger.info("Starting UDT-X Encrypted-Session Anomaly Detection Engine")
    ...
    try:
        ...
        logger.info("Shutting down Encrypted-Session Anomaly engine.")
        sys.exit(0)
