"""UDT-X C2 Beaconing Detection Engine CLI Entrypoint."""
...
logger = logging.getLogger("udtx.engines.c2_beacon.main")


def main() -> None:
    parser = argparse.ArgumentParser(description="UDT-X C2 Beaconing Detection Engine")
    parser.add_argument(
        "--kafka-brokers",
    ...
    parser.add_argument(
        "--group-id",
        default=os.getenv("KAFKA_GROUP_ID", "udtx-c2-beacon-engine"),
    )
    parser.add_argument(
        "--threshold",
        type=float,
        default=float(os.getenv("CONFIDENCE_THRESHOLD", "0.50")),
        help="Beacon confidence threshold for alert emission (0.0–1.0)",
    )
    parser.add_argument("--dry-run", action="store_true")
    args = parser.parse_args()

    logger.info("Starting UDT-X C2 Beaconing Detection Engine")
    ...
    try:
        ...
        logger.info("Shutting down C2 Beacon engine.")
        sys.exit(0)
