  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-6 bg-[#0B1220]/80 backdrop-blur-md animate-fade-in font-mono">
      <div className="w-full max-w-2xl h-[640px] rounded-2xl bg-[#131B2E] border border-[#3FC7D4]/30 shadow-[0_0_50px_rgba(63,199,212,0.2)] flex flex-col overflow-hidden relative">
            ...
            <div>
              <div className="flex items-center gap-2">
                ...
                <span className={`px-2 py-0.5 rounded text-[10px] font-bold flex items-center gap-1 ${
                  voiceState.engine === "wasm_offline"
                    ? "bg-[#FF8A3D]/15 border border-[#FF8A3D]/30 text-[#FF8A3D]"
                    : "bg-[#3FC7D4]/15 border border-[#3FC7D4]/30 text-[#3FC7D4]"
                }`}>
                  <Mic className="w-3 h-3" />
                  <span>{voiceState.engine === "wasm_offline" ? "VOICE: ON-DEVICE WASM" : "VOICE: WEB SPEECH"}</span>
                </span>
              </div>
        ...
        {/* Mid-Listen Network Transition Notification Banner */}
        {voiceState.handoffMessage && (
          <div className="px-4 py-2 bg-[#FF8A3D]/10 border-b border-[#FF8A3D]/30 text-[#FF8A3D] text-[11px] flex items-center justify-between animate-pulse">
            <div className="flex items-center gap-2">
              <span className="w-2 h-2 rounded-full bg-[#FF8A3D]" />
              <span>{voiceState.handoffMessage}</span>
            </div>
          </div>
        )}
