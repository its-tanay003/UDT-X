  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row items-start md:items-center justify-between pb-4 border-b border-[#3FC7D4]/15 gap-4">
        <div>
          ...
          <p className="text-xs text-[#8A95AA] mt-0.5">
            Real-time situational awareness, global enclave risk posture, and passive data diode telemetry.
          </p>
        </div>
        ...
        <div className="flex items-center gap-3">
          <Tooltip
            title="Wire Rate"
            code="EPS"
            content="Real-time flow events per second ingested passively across optical fiber taps."
          >
            <div className="flex items-center gap-2 px-3 py-1.5 rounded-lg bg-[#131B2E] border border-[#3FC7D4]/20 font-mono text-xs text-[#8A95AA] cursor-help hover:border-[#3FC7D4]/50 transition-colors">
              <Radio className="w-3.5 h-3.5 text-[#3FC7D4] animate-pulse" />
              <span>WIRE RATE:</span>
              {flowsPerSec !== null ? (
                <span className="text-[#3FC7D4] font-bold">
                  <AnimatedNumber value={flowsPerSec} /> EPS
                </span>
              ) : (
      ...
      <div id="tour-kpis" className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <Tooltip
          title="Sustained Flow Rate"
          code="FLOWS/SEC"
          content="Total unidirectional network flows processed by the normalizer and sliding feature windows."
          className="w-full"
        >
          <div id="tour-kpi-wire" className="p-5 rounded-xl bg-[#131B2E] hover:bg-[#1B2540] border border-[#3FC7D4]/15 hover:border-[#3FC7D4]/40 transition-all duration-200 hover:-translate-y-0.5 active:scale-[0.99] relative overflow-hidden flex flex-col justify-between w-full h-full cursor-help shadow-lg hover:shadow-[0_0_20px_rgba(63,199,212,0.15)]">
              ...
              {flowsPerSec !== null ? (
                <>
                  <span className="text-3xl font-display font-bold text-[#E7ECF5]">
                    <AnimatedNumber value={flowsPerSec} />
                  </span>
        ...
        <Tooltip
          ...
          code="SESSIONS"
          content="Ongoing suspicious network sessions currently exceeding Gaussian baseline thresholds."
          className="w-full"
        >
          <div id="tour-kpi-threats" className="p-5 rounded-xl bg-[#131B2E] hover:bg-[#1B2540] border border-[#FF8A3D]/25 hover:border-[#FF8A3D]/50 transition-all duration-200 hover:-translate-y-0.5 active:scale-[0.99] relative overflow-hidden flex flex-col justify-between w-full h-full cursor-help shadow-lg hover:shadow-[0_0_20px_rgba(255,138,61,0.15)]">
            <div className="flex items-center justify-between text-[#8A95AA] text-xs font-mono">
              ...
              {activeThreats !== null ? (
                <>
                  <span className="text-3xl font-display font-bold text-[#FF8A3D]">
                    <AnimatedNumber value={activeThreats} />
                  </span>
        ...
        <Link to="/app/incidents" className="w-full">
          <Tooltip
            ...
            code="CORRELATION"
            content="Multi-stage attack sequences correlated by graph proximity and temporal clustering within a 30m window."
            className="w-full"
          >
            <div id="tour-kpi-incidents" className="p-5 rounded-xl bg-[#131B2E] hover:bg-[#1B2540] border border-[#FF4757]/30 hover:border-[#FF4757]/60 transition-all duration-200 hover:-translate-y-0.5 active:scale-[0.99] relative overflow-hidden flex flex-col justify-between w-full h-full cursor-pointer shadow-lg hover:shadow-[0_0_20px_rgba(255,71,87,0.15)]">
              <div className="flex items-center justify-between text-[#8A95AA] text-xs font-mono">
              ...
              <div className="text-[11px] font-mono text-[#3FC7D4] flex items-center justify-between">
                <span>Neo4j 30m window mapped</span>
                <span>VIEW →</span>
              </div>
            </div>
          </Tooltip>
        ...
        <Link to="/app/alerts" className="w-full">
          <Tooltip
            ...
            code="RATE"
            content="Total security alerts evaluated and stored per minute, mapped to MITRE ATT&CK techniques."
            className="w-full"
          >
            <div id="tour-kpi-alerts" className="p-5 rounded-xl bg-[#131B2E] hover:bg-[#1B2540] border border-[#3FC7D4]/15 hover:border-[#3FC7D4]/50 transition-all duration-200 hover:-translate-y-0.5 active:scale-[0.99] relative overflow-hidden flex flex-col justify-between w-full h-full cursor-pointer shadow-lg hover:shadow-[0_0_20px_rgba(63,199,212,0.15)]">
              <div className="flex items-center justify-between text-[#8A95AA] text-xs font-mono">
              ...
              <div className="my-3">
                {alertsPerMin !== null ? (
                  <>
                    <span className="text-3xl font-display font-bold text-[#E7ECF5]">
                      <AnimatedNumber value={alertsPerMin} />
                    </span>
              ...
              <div className="text-[11px] font-mono text-[#3FC7D4] flex items-center justify-between">
                <span>100% MITRE ATT&CK tagged</span>
                <span>VIEW →</span>
              </div>
            </div>
          </Tooltip>
        ...
        <div id="tour-sphere-panel" className="lg:col-span-7 rounded-xl bg-[#131B2E] border border-[#3FC7D4]/20 p-5 flex flex-col justify-between">
          <div className="flex items-center justify-between mb-3">
            <div className="flex items-center gap-2">
              ...
              <Tooltip
                title="3D Listening Post"
                content="WebGL visual representation of network nodes communicating strictly inward across the passive tap."
              >
        ...
        <div id="tour-risk-panel" className="lg:col-span-5 rounded-xl bg-[#131B2E] border border-[#3FC7D4]/20 p-5 flex flex-col items-center justify-between">
          <div className="w-full flex items-center justify-between">
            ...
            <Tooltip
              ...
              code="0-100"
              content="Enclave-wide hazard score calculated from active incident stages and statistical variance."
            >
              <HelpCircle className="w-3.5 h-3.5 text-[#8A95AA] hover:text-[#3FC7D4] cursor-help" />
            </Tooltip>
          </div>
      ...
      <div id="tour-engines-grid" className="rounded-xl bg-[#131B2E] border border-[#3FC7D4]/15 p-5">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-7 gap-3 font-mono text-xs">
          {[
            { name: "5a DDoS Surge", desc: "Volumetric Rate", tip: "High-rate SYN flood & packet volume surge detector" },
            { name: "5b Recon Scan", desc: "Port Sweep/SYN", tip: "Horizontal & vertical port scan detection engine" },
            { name: "5c C2 Beacon", desc: "IAT Periodicity", tip: "Command & control heartbeat jitter & periodicity analysis" },
            { name: "5d DGA/Tunnel", desc: "Entropy & Base32", tip: "Domain generation algorithm and DNS tunneling decoder" },
            { name: "5e TLS Anomaly", desc: "JA3 Fingerprint", tip: "Encrypted session anomalies and malicious JA3 hashes" },
            { name: "5f Exfiltration", desc: "Asymmetric Out", tip: "Outbound payload byte volume and destination novelty detector" },
            { name: "ML TreeSHAP", desc: "LightGBM Model", tip: "Supervised gradient-boosted decision trees with local SHAP attributions" },
          ].map((eng, idx) => (
            <Tooltip key={idx} title={eng.name} content={eng.tip} className="w-full">
              <div
                className="p-3 rounded-lg bg-[#0B1220] border border-[#3FC7D4]/10 hover:border-[#3FC7D4]/40 transition-colors flex flex-col justify-between w-full h-full cursor-help"
              >
                ...
                <div className="flex items-center justify-between mt-2">
                  ...
                  <span className="text-[10px] text-[#4CAF7D] font-bold">● ONLINE</span>
                </div>
              </div>
            </Tooltip>
          ))}
